 ...
 void Server::start() {
     ...
     while (true) {
         if (this->role == ServerRole::LEADER) {
             ...
             runAsLeader();
         } else { // this->role == ServerRole::BACKUP
...
void Server::receiveNumbers() {
    ...
    while (this->role == ServerRole::LEADER) {
        ...
        if (received > 0 && number.type == Type::REQ) {
            ...
            bool isDuplicate = isDuplicateRequest(clientIP, number.seq);
            ...
            if (isDuplicate) {
                ...
                // ** INÍCIO DA LÓGICA DE REPLICAÇÃO **
                log_with_timestamp("[" + my_ip + "] Pedido processado localmente. Iniciando replicação para backups...");
                // Passa o endereço do cliente original para a função de replicação
                bool replicated = replicateToBackups(number, clientAddr);
                if (!replicated) {
                    log_with_timestamp("[" + my_ip + "] AVISO: A replicação para um ou mais backups falhou ou expirou.");
                } else {
                    log_with_timestamp("[" + my_ip + "] Replicação concluída com sucesso.");
                }
            }

            // Envia a confirmação ao cliente APÓS tentar a replicação
...
// Função modificada para gerir a replicação passiva
bool Server::replicateToBackups(const Message& client_request, const struct sockaddr_in& client_addr) {
    lock_guard<mutex> lock(serverListMutex);
    if (server_list.empty()) {
        return true; // Não há backups para replicar, considera-se sucesso.
    }
    ...
    // Adiciona o IP do cliente original à mensagem de replicação
    ...
    thread ack_listener([&]() {
        ...
        while(chrono::steady_clock::now() < end_time && acks_received < backups_count) {
            Message response;
            if(recvfrom(this->server_socket, &response, sizeof(response), 0, NULL, NULL) > 0) {
        ...
        if (received > 0) {
            switch(msg.type) {
                ...
                case Type::REPLICATION_UPDATE:
                    {
                        // Extrai o IP do cliente original do corpo da mensagem
                        ...
                        log_with_timestamp("["+my_ip+"] Recebida atualização de estado para o cliente " + client_ip_str);
                        
                        // O backup atualiza o seu próprio estado usando o IP correto
...
void Server::printParticipants(const std::string &clientIP)
{
 ...
 {
     string msg = " client " + clientIP +
                  " DUP !! id_req " + to_string(number.seq) +
                  " value " + to_string(number.num) +
                  " num_reqs " + to_string(sumTotal.num_reqs) +
                  " total_sum " + to_string(sumTotal.sum);
     log_with_timestamp(msg);
 }
 ...
 void Server::updateParticipant(const string &clientIP, uint32_t seq, uint32_t num)
 {
     ...
     participants.push_back({clientIP, num, seq, num});
 }
